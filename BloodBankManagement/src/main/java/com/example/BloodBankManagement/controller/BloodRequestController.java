...
public class BloodRequestController {
    ...
    public ResponseEntity<List<BloodRequest>> acceptBloodRequestByEmail(@PathVariable String id) {
        List<BloodRequest> acceptedBloodRequests = bloodRequestService.acceptBloodRequestByEmail(id);
        return new ResponseEntity<>(acceptedBloodRequests, HttpStatus.OK);
    }

    // @PreAuthorize("hasRole('ADMIN')")
    @PutMapping("/reject/{id}")
    public ResponseEntity<List<BloodRequest>> rejectBloodRequestByEmail(@PathVariable String id) {
        List<BloodRequest> rejectBloodRequests = bloodRequestService.rejectBloodRequestByEmail(id);
        return new ResponseEntity<>(rejectBloodRequests, HttpStatus.OK);
    }

    @PutMapping("/{requestId}")
    public ResponseEntity<BloodRequest> updateBloodRequest(@PathVariable String requestId,
